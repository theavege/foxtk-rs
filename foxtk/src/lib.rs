...
impl_widget!(ComboBox, IdExt, FrameExt, DrawableExt, WindowExt, PackerExt, CompositeExt);
impl ComboBox {
    pub fn new(parent: &impl CompositeExt, cols: i32) -> Self {
        Self::from_raw(unsafe { FXComboBox_new(parent.as_raw() as *mut FXComposite, cols) })
            .with_layout(Layout::FillX)
    }
}
...
impl_widget!(Spinner, IdExt, FrameExt, DrawableExt, WindowExt, CompositeExt, PackerExt);
impl Spinner {
    pub fn new(parent: &impl CompositeExt) -> Self {
        Self::from_raw(unsafe { FXSpinner_new(parent.as_raw() as *mut FXComposite) })
            .with_layout(Layout::FillX)
