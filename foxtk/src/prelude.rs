...
/// Converts a string to CString, returning an error if it contains null bytes.
/// This prevents silent failures and helps catch bugs early.
pub(crate) fn to_cstring(s: &str) -> Result<CString, String> {
    if s.contains('\0') {
        Err(format!("String contains null byte: {:?}", s))
    } else {
        Ok(CString::new(s).unwrap())
    }
}
    ...
    fn open_file_dialog(&self, caption: &str, path: &str, patterns: &str, initial: i32) -> String {
        unsafe {
            let ptr = FXFileDialog_get_open_filename(
                ...
                to_cstring(caption).unwrap().as_ptr(),
                to_cstring(path).unwrap().as_ptr(),
                to_cstring(patterns).unwrap().as_ptr(),
                initial,
            );
        ...
        unsafe {
            match kind {
                Message::Error => FXMessageBox_error(
                    self.root().as_raw() as *mut FXWindow,
                    opts as u32,
                    to_cstring("Error").unwrap().as_ptr(),
                    to_cstring(message).unwrap().as_ptr(),
                ),
                Message::Information => FXMessageBox_information(
                    self.root().as_raw() as *mut FXWindow,
                    opts as u32,
                    to_cstring("Information").unwrap().as_ptr(),
                    to_cstring(message).unwrap().as_ptr(),
                ),
                Message::Question => FXMessageBox_question(
                    self.root().as_raw() as *mut FXWindow,
                    opts as u32,
                    to_cstring("Question").unwrap().as_ptr(),
                    to_cstring(message).unwrap().as_ptr(),
                ),
                Message::Warning => FXMessageBox_warning(
                    self.root().as_raw() as *mut FXWindow,
                    opts as u32,
                    to_cstring("Warning").unwrap().as_ptr(),
                    to_cstring(message).unwrap().as_ptr(),
                ),
            }
        }
